...
initial_prompt = (
 ...
 "학생이 LaTeX 형식으로 수학 수식을 입력할 수 있으므로, 이를 자동으로 인식하여 수식 형태로 렌더링하세요. 또한 당신이 학생에게 수학 개념이나 공식을 안내할 때도 LaTeX 수식으로 시각적으로 표현하여 전달하세요. LaTeX 수식은 `\\[ ... \\]` 또는 `$...$`로 감싸서 출력하고, 가능한 경우 렌더링되도록 하세요. 단, LaTeX 문법에 오류가 있을 경우 오류 메시지를 출력하지 말고, 자연스럽게 올바른 수식 표현을 안내하며 유도하세요."
