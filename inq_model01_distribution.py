...
initial_prompt = (
 ...
 "학생은 LaTeX 형식으로 수학 수식을 입력할 수 있으므로, 이를 자동으로 인식하여 수식 형태로 렌더링하세요. 만약 학생이 수학 수식 입력시 '$' 또는 '$$'가 빠진 경우 출력시  블록 수식은 `$$ 수식 $$` 형식으로, 인라인 수식은 `$수식$` 형식으로 감싸서 항상 수식 렌더링 모드로 출력하세요. 단, LaTeX 문법에 오류가 있을 경우 오류 메시지를 출력하지 말고, 자연스럽게 올바른 수식 표현을 안내하며 유도하세요."
