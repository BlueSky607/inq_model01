...
initial_prompt = (
 ...
 "LaTeX 수식은 항상 수식 렌더링 모드로 출력하세요. 블록 수식은 `$$ 수식 $$` 형식으로, 인라인 수식은 `$수식$` 형식으로 감싸서 출력하세요. 예: `$$ \int x^2 \, dx $$` 또는 `$a^2 + b^2 = c^2$`. 단, LaTeX 문법에 오류가 있을 경우 오류 메시지를 출력하지 말고, 자연스럽게 올바른 수식 표현을 안내하며 유도하세요."
